...
void MeshData::generateVertexLink()
{
	...
	Node** vertex_link;
	//allocate memory
	vertex_link = new Node*[vertex_N];
	int i;  for(i = 0; i<vertex_N; i++)
		vertex_link[i] = new Node;

	for(i=0; i<face_N; i++){
		if(face[i][0] < 0)
			continue;

		vertex_link[face[i][0]]->append(face[i][1], i);
		vertex_link[face[i][0]]->append(face[i][2], i);

		vertex_link[face[i][1]]->append(face[i][2], i);
		vertex_link[face[i][1]]->append(face[i][0], i);

		vertex_link[face[i][2]]->append(face[i][0], i);
		vertex_link[face[i][2]]->append(face[i][1], i);
	}
	*/

	int* link_N = new int[vertex_N];  //link_N[]为顶点的三角形片面的数目数组
	int i;  for(i = 0; i<vertex_N; i++)
		link_N[i] = 0;
	//排除无效的面
	for(i=0; i<face_N; i++){
	...
	int** link = new int*[vertex_N];  //link[][]为某顶点的片面链表
	for(i=0; i<vertex_N; i++){
	...
	for(i=0; i<face_N; i++){
		...
		int j;  for(i = 0; j<3; j++){
			...
			if(index < 0)
				continue;
			link[index] [3*link_N[index]] = face[i][(j+1)%3];  //=三角片面的另外一个顶点
			link[index] [3*link_N[index]+1] = face[i][(j+2)%3];  //=三角片面的另外一个顶点
			link[index] [3*link_N[index]+2] = i;  //=三角片面的编号
			link_N[index]++;
		}
	}
	...
	for(i=0; i<vertex_N; i++){
		...
		if(link_N[i] != 0)
			delete[] link[i];
		/*
		isBound[i] = this->sortVertexLink(vertex_link[i], vertex_link_v[i], vertex_link_f[i], 
										  degree_v[i], degree_f[i]);
										  */
	}
	/*
	for(i=0; i<vertex_N; i++){
		delete vertex_link[i];
	}*/
	delete[] link;
	delete[] link_N;
	//delete[] vertex_link;
...
void MeshData::computeFaceNormal2()
{
	if(normal_face != NULL)
		delete[] normal_face;
	normal_face = new float[face_N][3];

	int i;
	//FILE * file;
	//file = fopen("d://tmp.txt", "w");
	for(i=0; i<face_N; i++){
		int *f = face[i];
		float v1[3], v2[3];
		double c[3];
		...
		normal_face[i][0] = (float)(c[0]/length);
		normal_face[i][1] = (float)(c[1]/length);
		normal_face[i][2] = (float)(c[2]/length);
		//fprintf(file, "%f  %f  %f\n", normal_face[i][0], normal_face[i][1], normal_face[i][2]);
	}
	//fclose(file);
}

void MeshData::computeArea(float area_limit)
{
	//compute the area of the face
	if(area != NULL)
		delete[] area;
	area = new float[face_N];
	isBig = new BOOL[face_N];
	
	double (*tmp)[3] = new double[face_N][3];
	normal_face = new float[face_N][3];
	int i = 0;
	int j = 0;
	int k = 0;
	int sum = 0;
	int sumbig = 0;
	//int area_limit = 100;  //参数1，设置取样域
	//FILE * file;
	//file = fopen("d://tmp5.txt", "w");
	for(i=0; i<face_N; i++){
		int *f = face[i];
		area[i] = AREA(vertex[f[0]], vertex[f[1]], vertex[f[2]]);
	}
	float area_min = *std::min_element(area, area + face_N);
	float area_max = *std::max_element(area, area + face_N);
	float area_limit_compare = (area_max / area_min) / 100.0 * area_limit;
	for(i = 0; i < face_N; i++) {
		if(area[i] / area_min > area_limit_compare) {
			isBig[i] = TRUE;
		} else {
			isBig[i] = FALSE;
		}
	}
	int * a1 = new int[face_N];
	for(i = 0; i < face_N; i++) {
		a1[i] = area[i] / (area_limit_compare * area_min);
	}
	for(i = 0; i < face_N; i++) {
		if(isBig[i] == FALSE) {
			continue;
	...
	//fprintf(file, "\n-------------------------------\n");
	//fprintf(file, "sum = %d\nk = %d\nsumbig = %d\n", sum, k, sumbig);
	sample_array = new int[k];
	//sample_array = (int *)malloc(sizeof(int) * sum);
	//memset(sample_array, 0, sum);
	//int testb[30];
	...
	for(i = 0; i < k; i++) {
		sample_array[i] = a2[i];
		//fprintf(file, "sample  %d  %d\n", i, sample_array[i]);
		//fprintf(file, "a2  %d  %d\n", i, a2[i]);
	}
	//fprintf(file, "////%f    %f    %d    %d\n\n", area_min, area_max, sum, sumbig);
	for(i = 0; i < k; i++) {
		// 打印面的序号，面的三个顶点，面的法向量
		//fprintf(file, "%d %d %d %d %f %f %f\n", a2[i], tmp[a2[i]][0], tmp[a2[i]][1], tmp[a2[i]][2], normal_face[a2[i]][0], normal_face[a2[i]][1], normal_face[a2[i]][2]);
	}
	//fclose(file);
...
void MeshData::computePrincipal()
{
	...
	for(int i = 0; i<vertex_N; i++){
		int nei_N, *nei;  //nei_N = degree_v[index]; nei = vertex_link_v[index];
		getConsistent1Ring(i, nei, nei_N);  //得到邻居顶点
		if(nei_N == 0){
			k_max[i] = k_min[i] = 0;
		...
		for(int m=0; m<nei_N; m++){
			...
			//directinal curvature  方向曲率
			
			double len2 = v[0]*v[0]+v[1]*v[1]+v[2]*v[2];
			if((float)len2 == 0)
				continue;
			double ki = 2.0*DOT(v,normal[i])/len2;  //DOT为向量点乘
...
int MeshData::sortVertexLink(Node *link, int *&link_v, int *&link_f, int &v, int &f)
{
	//count incident face
	f = 0;
	for(Node* current=link; current->next!=NULL; current=current->next->next)
		f++;
...
void MeshData::copyMesh2(MeshData *mesh)
{
	...
	setVertexCount(v_N);
	int counter = 0;
	int i;  for(i = 0; i<mesh->vertex_N; i++){
...
void MeshData::generateSamplePointRandamly(int sample_N)
{
	this->sample_N = sample_N;
	sample_point = new tri_point[sample_N];
	t_max_line_N = new int[sample_N];
	t_min_line_N = new int[sample_N];
	t_max_line = new tri_point*[sample_N];
	t_min_line = new tri_point*[sample_N];

	float *area = new float[face_N];
	int *index = new int[face_N];
	int i;  for(i = 0; i<face_N; i++){
		area[i] = (float)faceArea(i);
		index[i] = i;
	}
	quickSort(index, area, 0, face_N-1);
	float total = 0;
	for(i=0; i<face_N; i++){
		total += area[i];
		area[i] = total;
	...
	srand(100);
	for(i=0; i<sample_N; i++){
		double ran = (double)total*rand()/RAND_MAX;
		int j;  for(i = 0; j<face_N-1; j++)
			if(ran < area[j])
				break;

		sample_point[i].id = index[j];
		sample_point[i].u = (double)rand()/RAND_MAX;
		sample_point[i].v = (double)rand()*(1.0-sample_point[i].u)/RAND_MAX;

		//traceTmax(sample_point[i], 1.0f, t_max_line[i], t_max_line_N[i], 0.01f);
		//traceTmin(sample_point[i], 1.0f, t_min_line[i], t_min_line_N[i], 0.01f);

		traceTmax2(sample_point[i], 1.0f, t_max_line[i], t_max_line_N[i], 0.01f, 0.8f);
		traceTmin2(sample_point[i], 1.0f, t_min_line[i], t_min_line_N[i], 0.01f, 0.8f);
	}
}

void MeshData::generateSamplePointRandamly2(int sample_N)
{
	if(sample_point != NULL){
		delete[] sample_point;
