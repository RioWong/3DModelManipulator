...
public:
	...
	void generateSamplePointRandamly2(int smaple_N);
	void generateSamplePointRandamly(int smaple_N);
	...
	void computeNormal();
	void computeArea(float area_limit);
	void computeFaceNormal();
	void computeFaceNormal2();
	...
	static inline void MeshData::SAMPLE(float s[3], float a[3], float b[3], float c[3]) {
		//srand (time(NULL));
		float u = rand() % 100 / 100.0;
		float v = rand() % 100 / 100.0;
		if(u + v > 1) {
			u = 1 - u;
			v = 1 - v;
		}
		float w;
		w = 1 - u - v;
		s[0] = u * a[0] + v * b[0] + w * c[0];
		s[1] = u * a[1] + v * b[1] + w * c[1];
		s[2] = u * a[2] + v * b[2] + w * c[2];
	}
